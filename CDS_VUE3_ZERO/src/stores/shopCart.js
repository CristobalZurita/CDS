...
export const useShopCartStore = defineStore('shop-cart', () => {
  ...
  const currentShipping = computed(() =>
    shippingOptions.find((option) => option.key === selectedShippingKey.value) || shippingOptions[0]
  )

  const itemsCount = computed(() =>
    items.value.reduce((sum, item) => sum + normalizeQty(item.qty), 0)
  )

  const totals = computed(() => {
    const productsSubtotal = items.value.reduce(
  ...
  function syncCatalog(products) {
    ...
    items.value = items.value
      .map((item) => {
        ...
        if (!product || !product.store_visible || product.enabled === false) return null
  ...
  function addProduct(product) {
    ...
    if (currentIndex < 0) {
      if (!canAddProduct(normalized)) return false
    ...
    if (nextQty <= normalizeQty(current.qty)) return false
  ...
  async function submitRequest(notes = '') {
    if (!items.value.length) throw new Error('El carrito está vacío')
  ...
  watch(items, (value) => { persistJson(CART_STORAGE_KEY, value) }, { deep: true })
  watch(selectedShippingKey, (value) => { persistJson(SHIPPING_STORAGE_KEY, value) })
