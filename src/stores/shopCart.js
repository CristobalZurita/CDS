...
export const useShopCartStore = defineStore('shop-cart', () => {
  ...
  const itemsCount = computed(() => items.value.reduce((sum, item) => sum + normalizeQty(item.qty), 0))
    ...
    try {
      ...
      if (raw) {
        ...
        if (Array.isArray(parsed)) {
          items.value = parsed.map(normalizeCartItem).filter(Boolean)
  ...
  function replaceItems(nextItems) {
    items.value = nextItems.map(normalizeCartItem).filter(Boolean)
  }

  function syncCatalog(products) {
    const catalog = new Map(
      (Array.isArray(products) ? products : [])
        .map((product) => normalizeCartItem({ ...product, qty: 1 }))
        .filter(Boolean)
  ...
  function canAddProduct(product) {
    ...
    const current = items.value.find((item) => String(item.id) === String(normalized.id))
  ...
  function addProduct(product) {
    ...
    const currentIndex = items.value.findIndex((item) => String(item.id) === String(normalized.id))
    if (currentIndex < 0) {
    ...
    nextItems[currentIndex] = normalizeCartItem({ ...current, ...normalized, qty: nextQty })
  ...
  function changeQty(productId, delta) {
    const index = items.value.findIndex((item) => String(item.id) === String(productId))
    if (index < 0) return
    ...
    const nextItems = [...items.value]
    nextItems[index] = normalizeCartItem({ ...current, qty: nextQty })
    items.value = nextItems
  }
  ...
  function buildRequestPayload(notes = '') {
    return {
  ...
  async function submitRequest(notes = '') {
    ...
    try {
      const res = await api.post('/client/store/purchase-requests', buildRequestPayload(notes))
