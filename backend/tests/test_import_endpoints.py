import sys
import os
import time
import sqlite3
import asyncio
import pytest

ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
if ROOT not in sys.path:
    sys.path.insert(0, ROOT)

from fastapi.testclient import TestClient
from fastapi import HTTPException, status
from fastapi import BackgroundTasks
from app.main import app
from app.core.dependencies import get_current_admin
from app.api.v1.endpoints.imports import DB_PATH, create_import

def test_create_import_run_unauthenticated():
    # No auth header -> 403 (HTTPBearer returns 403 when no credentials)
    with TestClient(app) as client:
        r = client.post('/api/v1/imports/run')
        assert r.status_code == status.HTTP_403_FORBIDDEN


def test_create_import_run_forbidden_non_admin():
    with pytest.raises(HTTPException) as exc:
        asyncio.run(get_current_admin({"user_id": "2", "role": "client"}))

    assert exc.value.status_code == status.HTTP_403_FORBIDDEN


def test_create_import_run_admin():
    data = create_import(BackgroundTasks(), {"user_id": "1", "role": "admin"})
    assert data.get('status') == 'started'
    assert 'run_id' in data
    run_id = data.get('run_id')
    # verify an audit_logs entry was recorded for the request
    db_path = DB_PATH
    assert os.path.exists(db_path)
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
    cur.execute('SELECT count(*) FROM import_runs WHERE run_id = ?', (run_id,))
    assert cur.fetchone()[0] == 1
    conn.close()


def test_import_runs_exist():
    db_path = DB_PATH
    assert os.path.exists(db_path)
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
    cur.execute('SELECT count(*) FROM import_runs')
    cnt = cur.fetchone()[0]
    conn.close()
    assert cnt >= 1
